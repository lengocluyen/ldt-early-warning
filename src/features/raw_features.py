...
from src.ldt_pipeline import load_config, load_oulad, preprocess, raw_features, split_presentations


def main() -> None:
    parser = argparse.ArgumentParser(description="Compute conventional raw-table features.")
    ...
    frame = raw_features(tables, args.module, presentations, args.checkpoint)
    output = Path(args.output or f"results/features/{args.module}_{args.partition}_raw_day{args.checkpoint}.csv")
